      ...
      @else 
        <form method="POST" action="{{ url('/transaksi/'.$transaksi->penjualan_id) }}" class="form-horizontal"> 
            ...
            <div class="col-11"> 
              <select class="form-control" id="user_id" name="user_id" required> 
                ...
                @foreach($user as $item) 
                  <option value="{{ $item->user_id }}" @if($item->user_id == $transaksi->user_id) selected @endif>{{ $item->nama }}</option> 
                @endforeach 
              </select> 
              @error('user_id') 
          ...
          <div class="form-group row"> 
            ...
            <div class="col-11"> 
              <input type="datatime" class="form-control" id="penjualan_tanggal" name="penjualan_tanggal" value="{{ old('penjualan_tanggal',$transaksi->penjualan_tanggal)}}"> 
